...
if(Input::checkInput('request','post','1')){
	...
	switch($post_request){
       ...
       case 'entry-new':
            $form = NailEntryController::add();
            if($form->ERRORS == false){
                Session::put('success','Entry added successfully');
                Redirect::to(DNADMIN.'/app/entries/list');
            }else{
                // echo errors
            }
        break;
        ...
        case 'entry-status':
            Functions::flashMsg();
            $entry_ID = Input::get('entry-id', 'post');
            
            if (Input::checkInput('completed', 'post', '0')) {
                $status = 'completed';
                $form = NailEntryController::changeStatus($status, $entry_ID);
            } elseif (Input::checkInput('paid', 'post', '0')) {
                $status = 'paid';
                $form = NailEntryController::changeStatus($status, $entry_ID);
            } elseif (Input::checkInput('pending', 'post', '0')) {
                $status = 'pending';
                $form = NailEntryController::changeStatus($status, $entry_ID);
            } elseif (Input::checkInput('cancelled', 'post', '0')) {
                $status = 'cancelled';
                $form = NailEntryController::changeStatus($status, $entry_ID);
            }
       ...
       case 'entry-edit':
            $entry_ID = Input::get('entry-id', 'post');
            $form = NailEntryController::edit($entry_ID);
            if ($form->ERRORS == false) {
                Session::put('success', 'Entry updated successfully');
                Redirect::to(DNADMIN . '/app/entries/list');
            } else {
                Session::put('errors', $form->ERRORS_SCRIPT);
                Redirect::to(DNADMIN . '/app/entries/list');
            }
        break;
    
        // LIQUIDATION
        case 'liquidate-worker':
            Functions::flashMsg();
            $form = LiquidationController::create();
            if($form->ERRORS == false){
                Session::put('success','Liquidation created successfully');
                Redirect::to(DNADMIN.'/dashboard');
            }else{
                Session::put('errors', $form->ERRORS_SCRIPT);
                Redirect::to(DNADMIN.'/dashboard');
            }
        ...
        case 'liquidation-status':
            Functions::flashMsg();
            $liquidation_ID = Input::get('liquidation-id', 'post');
            
            if (Input::checkInput('paid', 'post', '0')) {
                $status = 'paid';
                $form = LiquidationController::changeStatus($status, $liquidation_ID);
            } elseif (Input::checkInput('pending', 'post', '0')) {
                $status = 'pending';
                $form = LiquidationController::changeStatus($status, $liquidation_ID);
            } elseif (Input::checkInput('cancelled', 'post', '0')) {
            ...
            Redirect::to(DNADMIN . '/app/liquidations/list');
        break;

        // BOOKINGS

        case 'booking-new':
            $form = BookingController::create();
            
            if(isset($form->HAS_OVERLAP) && $form->HAS_OVERLAP === true){
                // Store overlap data in session
                Session::put('overlap_data', $form->OVERLAP_DATA);
                Session::put('overlap_booking_id', $form->EXISTING_BOOKING_ID);
                Session::put('booking_form_data', $_POST); // Store form data
                Redirect::to(DNADMIN.'/app/booking/new?overlap=1');
            } elseif($form->ERRORS == false){
                Session::put('success','Booking created successfully');
                Redirect::to(DNADMIN.'/app/booking/list');
            }else{
                if(is_array($form->ERRORS_SCRIPT)) {
                    Session::put('errors', $form->ERRORS_SCRIPT);
                } else {
                    Session::put('errors', array($form->ERRORS_SCRIPT));
                }
                Redirect::to(DNADMIN.'/app/booking/new');
            }
        break;

        case 'booking-update':
            Functions::flashMsg();
            $booking_id = Input::get('booking_id', 'post');
            $form = BookingController::update($booking_id);
            if($form->ERRORS == false){
        ...
        case 'booking-create-entry':
            Functions::flashMsg();
            $booking_id = Input::get('booking_id', 'post');
            $form = BookingController::createEntry($booking_id);
            Redirect::to(DNADMIN.'/app/booking/list');
        break;
        ...
        case 'booking-cancel':
            Functions::flashMsg();
            $booking_id = Input::get('booking_id', 'post');
            $form = BookingController::cancelBooking($booking_id);
            Redirect::to(DNADMIN . '/app/booking/list');
        break;
        ...
        // SERVICES
        case 'service-new':
    $form = ServiceController::add();
    if($form->ERRORS == false){
        Session::put('success','Service added successfully');
        Redirect::to(DNADMIN.'/app/services/list');
    }else{
        // Handle errors - you can display them on the form
        if(isset($form->ERRORS_SCRIPT) && is_array($form->ERRORS_SCRIPT)){
            foreach($form->ERRORS_SCRIPT as $error){
                if($error != 'ERRORS_FOUND'){
...
case 'service-edit':
    $form = ServiceController::update();
    if($form->ERRORS == false){
        Session::put('success','Service updated successfully');
        Redirect::to(DNADMIN.'/app/services/list');
    }else{
        // Handle errors
        if(isset($form->ERRORS_SCRIPT) && is_array($form->ERRORS_SCRIPT)){
...
case 'service-status':
    Functions::flashMsg();
    $service_ID = Input::get('service-id', 'post');
    
    $activate = Input::get('activate', 'post');
    $deactivate = Input::get('deactivate', 'post');
    
    if ($activate == '1') {
        $status = 'active';
        $form = ServiceController::changeStatus($status, $service_ID);
    } elseif ($deactivate == '1') {
        $status = 'inactive';
        $form = ServiceController::changeStatus($status, $service_ID);
    }
...
case 'service-delete':
    Functions::flashMsg();
    $service_ID = Input::get('service-id', 'post');
    $form = ServiceController::delete($service_ID);
    Redirect::to(DNADMIN . '/app/services/list');
break;
...
// =====================================================
// EXPENSE MANAGEMENT CASES
// Add these cases to your main controller file
// =====================================================

case 'expense-new':
    Functions::flashMsg();
    try {
        $form = ExpenseController::create();
        if ($form && $form->ERRORS) {
            Session::put('errors', $form->ERRORS_SCRIPT);
        }
    } catch (Throwable $e) {
        Session::put('errors', 'Error: ' . $e->getMessage());
    }
    Redirect::to(DNADMIN . '/app/expenses/list');
break;

case 'expense-update':
    Functions::flashMsg();
    try {
        $expense_ID = Input::get('expense-id', 'post');
        $form = ExpenseController::update($expense_ID);
        if ($form && $form->ERRORS) {
            Session::put('errors', $form->ERRORS_SCRIPT);
        }
    } catch (Throwable $e) {
...
case 'expense-liquidate':
    Functions::flashMsg();
    try {
        $expense_ID = Input::get('expense-id', 'post');
        $form = ExpenseController::convertToLiquidation($expense_ID);
        if ($form && $form->ERRORS) {
            Session::put('errors', $form->ERRORS_SCRIPT);
        }
    } catch (Throwable $e) {
...
case 'expense-delete':
    ...
    try {
        $expense_ID = Input::get('expense-id', 'post');
        $form = ExpenseController::delete($expense_ID);
        if ($form && $form->ERRORS) {
            Session::put('errors', $form->ERRORS_SCRIPT);
        }
    } catch (Throwable $e) {
...
case 'expense-status':
    ...
    try {
        $expense_ID = Input::get('expense-id', 'post');
        $pending    = Input::get('pending',    'post');
        $waiting    = Input::get('waiting',    'post');
        $liquidated = Input::get('liquidated', 'post');
        $cancelled  = Input::get('cancelled',  'post');

        if ($pending    == '1') ExpenseController::changeStatus('pending',    $expense_ID);
        elseif ($waiting    == '1') ExpenseController::changeStatus('waiting',    $expense_ID);
        elseif ($liquidated == '1') ExpenseController::changeStatus('liquidated', $expense_ID);
        elseif ($cancelled  == '1') ExpenseController::changeStatus('cancelled',  $expense_ID);
    } catch (Throwable $e) {
        Session::put('errors', 'Error: ' . $e->getMessage());
    }
    Redirect::to(DNADMIN . '/app/expenses/list');
...
// =====================================================
// EXAMPLE: How to get expense data in other pages
// =====================================================

/*
// Get all expenses
$all_expenses = ExpenseController::getAllExpenses();

// Get expenses with filters
$filters = array(
    'user_id' => 5,                    // Specific worker
    'expense_type' => 'salon_supply',  // Type filter
    'status' => 'pending',             // Status filter
    'from_date' => '2026-02-01',       // Date range
    'to_date' => '2026-02-28',
    'keyword' => 'nail polish'         // Search keyword
);
$filtered_expenses = ExpenseController::getAllExpenses($filters);

// Get expense statistics
$stats = ExpenseController::getExpenseStats('2026-02-01', '2026-02-28');
echo $stats->total_amount;
echo $stats->salon_supply_total;
echo $stats->internal_total;
echo $stats->external_total;

// Get worker's expense totals
$worker_expenses = ExpenseController::getWorkerExpenses($user_id);
echo $worker_expenses['pending'];
echo $worker_expenses['liquidated'];
echo $worker_expenses['total'];
*/
